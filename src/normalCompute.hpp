...
public:
	...
	void estimateNormal(GLMmodel* pModel)
	{
		...
		for (int i = 0; i <= pModel->numvertices; i++) {
			...
			std::vector<double> empty_i;
			pn.push_back(empty_i);

		}

		std::cout << "numtriangles:" << pModel->numtriangles << std::endl;
		std::cout << "numfaceNormal" << pModel->numfacetnorms << std::endl;
	...
	void estimateNormal(GLMmodel * pModel, bool accurate) {
		...
		std::vector<std::vector<int>> neiborTrans;//neibor tragular index
		std::vector<std::vector<int>> neiborTrans_index;//neibor tragular index
		std::vector<std::vector<double>> neiborTrans_Norm;//neibor trangular norm
		std::vector<std::vector<double>> neiborTrans_Area;//bneibor tragular area

		//add first item to regular data of norm and area
		std::vector<double> empty_int;
		...
		for (int i = 1; i < neiborTrans.size(); i++) {
			std::vector<int> tragular_index = neiborTrans[i];
			std::vector<int> tragular_index_index = neiborTrans_index[i];
			std::vector<double> norm_i;
			std::vector<double> area_i;
		...
		for (int i = 1; i <= pModel->numvertices; i++) {
			std::vector<double> norm_vi;
			norm_vi.push_back(0);
			norm_vi.push_back(0);
			norm_vi.push_back(0);
			std::vector<double> neiborTrans_Norm_i = neiborTrans_Norm[i];
			std::vector<double> neiborTrans_Area_i = neiborTrans_Area[i];
			double sum_area_i = 0;
	...
	std::vector<std::vector<double>> estimateNormal_PCL_MP_return(std::vector<std::vector<double>> pointsVector) {

		std::vector<std::vector<double>> normalVector2;
	...
	void estimateNormal_PCL_MP(std::vector<std::vector<double>> pointsVector) {
		...
		std::vector<std::vector<double>> nR = estimateNormal_RegularNormal(pointsVector, normalVector);
		normalVector.clear();
		normalVector = nR;
		normalSave(normalVector);
	}
	...
	bool normalLoad() {
		...
		if (fin)
		{
			...
			normalVector.clear();
			normalVector.reserve(numSum);
			std::vector<double> pn_i(3);
			for (int i = 0; i < numSum; i++) {
				std::cin >> pn_i[0] >> pn_i[1] >> pn_i[2];
				normalVector.push_back(pn_i);
			}
			return true;
		}
		else {
...
private:
	...
	double maxErrorNormalMeasure(std::vector<std::vector<double>> normalVector1, std::vector<std::vector<double>> normalVector2) {
		...
		for (int i = 0; i < normalVector1.size(); i++) {
			...
			std::vector<double> nv1 = normalVector1[i];
			std::vector<double> nv2 = normalVector2[i];
			std::vector<double> nv2_neg;
		...
		std::cout << "Single Point Num:" << singlePointNum << std::endl;
		return max;
	...
	void normalSave(std::vector<std::vector<double>> n) {

		if (fileNormal.size() == 2) {
			std::cout << "normal file name is empty!" << std::endl;
		}
		else {
			std::ofstream fout(fileNormal, std::ios::app);
			fout << n.size() << std::endl;
			for (int i = 0; i < n.size(); i++) {
				fout << n[i][0] << " " << n[i][1] << " " << n[i][2] << std::endl;
			}
			fout << std::endl;
			fout.close();
		}
	...
	std::vector<std::vector<double>> estimateNormal_RegularNormal(std::vector<std::vector<double>> pointCloudData, std::vector<std::vector<double>> pointNormal) {
