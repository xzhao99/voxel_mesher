...
/* Voronoi Diagram from CGAL */
class VoronoiD {

public:

	VD vd;
	std::vector<std::vector<double>> pOrignal;  //original points
	std::vector<std::vector<double>> halfEdage; //half Edage
	std::vector<std::vector<double>> rayEdage;  //ray Edage
	std::vector<std::vector<double>> halfEdage_r; //half Edage
	std::vector<std::vector<double>> rayEdage_r;  //ray Edage	
	std::vector<std::vector<double>> rayEdageBound; //ray Edage to bounding box
	std::vector<std::vector<double>> DelaunayTg; //Delaunay_triangulation
	
public:

	void VoronoiD_init(std::vector<std::vector<double>> pInput) {
	...
	std::vector<std::vector<double>> VoronoiD_Bounding(std::vector<std::vector<double>> pi, double r) {
		std::cout << "VoronoiD bounding run:" << std::endl;
		//store halfEdage and rayEdage
		std::vector<std::vector<double>> he1 = halfEdage;
		std::vector<std::vector<double>> re1 = rayEdage;
		...
		for (int i = 0; i < pi.size(); i++) {
			...
			std::vector<double> pi_i = pi[i];
			std::vector<std::vector<double>> pE = VoronoiD_Bounding_Single(pi_i, r);
			std::vector<double> finalResult_i;
		...
		halfEdage = he1;
		rayEdage = re1;
		std::cout << "VoronoiD bounding finished." << std::endl;
		return finalResult;		
	}
	
	std::vector<std::vector<double>> VoronoiD_Centroid(std::vector<std::vector<double>> bounding) {
		...
		for (int i = 0; i < bounding.size(); i++) {
			...
			std::vector<double> psite;
			std::vector<double> psiteWight;
			double Area = 0;
	...
	std::vector<double> VoronoiD_Centroid_Single(std::vector<double> pointC, double r) {

		std::vector<std::vector<double>> pointCB = VoronoiD_Bounding_Single(pointC, r);
		double x0 = pointC[0];
		double y0 = pointC[1];		
		std::vector<double> psite;
		std::vector<double> psiteWight;
	...
	std::vector<double> VoronoiD_Centroid_Single_meanPoints(std::vector<double> pointC, double r) {
	...
	std::vector<int> VoronoiD_Return_Delaunay(std::vector<double> pointC, std::vector<std::vector<double>> pointCN) {
		...
		std::vector<int> borderInt;

		for (int i = 0; i < border.size(); i++) {

			std::vector<double> border_p1;
			border_p1.push_back(border[i][0]);
			border_p1.push_back(border[i][1]);
			std::vector<double> border_p2;
	...
	std::vector<double> VoronoiD_Return_Delaunay_Mean(std::vector<double> pointC, std::vector<std::vector<double>> pointCN) {
...
private:
	...
	std::vector<std::vector<double>> VoronoiD_Bounding_Single(std::vector<double> pi, double r) {
	...
	std::vector<std::vector<double>> VoronoiD_Edge(std::vector<double> pointO, std::vector<double> rayStart,
		std::vector<double> rayContext, double r) {//achieve the next point of the edge
		//pointO: original point; rayStart: start point of ray; rayBefore: point have been selected
 		//computation the rayLine
		
		std::vector<double> rayStartStore;
		double min = 99999;
		for (int i = 0; i < rayEdageBound.size(); i++) {
			std::vector<double> rayStart_i = rayEdageBound[i];
			if (rayStart_i[0] == rayStart[0] && rayStart_i[1] == rayStart[1]) {
				std::vector<double> raySource_i;
		...
		min = 99999;
		std::vector<double> rayContextEndStore;
		for (int i = 0; i < rayEdageBound.size(); i++) {
			std::vector<double> rayStart_i = rayEdageBound[i];
		...
		if (rayContextEndStore.size() == 0 || (rayContextEndStore[0] == rayStartStore[0] &&
			...
			std::cout << "Error! the ray endpoint searching wrong!" << std::endl;

		}

		std::vector<std::vector<double>> finalResult;
		std::vector<double> p1, p2, p3;
		std::vector<double> p12, p23;
		...
		if (rayStartStore[0] == rayContextEndStore[0] || rayStartStore[1] == rayContextEndStore[1]) {			
			...
			if (xCrossRay == 0 || yCrossRay == 0) {
				std::cout << "Error! crossing point missing!" << std::endl;			
			}
	...
	int VoronoiD_search(std::vector<double> p, std::vector<std::vector<double>> pl) {

		for (int i = 0; i < pl.size(); i++) {
			std::vector<double> pli = pl[i];	// unused variable?
...
if (result[0][0] != result[result.size() - 1][2] &&
			...
			//not same line
				...
				std::vector<double> result_i1;
				std::vector<double> result_i2;
...
std::vector<double> VoronoiD_Edge(std::vector<double> pointO, std::vector<double> rayStart,
		vector<double> rayBefore, double r) {//achieve the next point of the edge
		//pointO: original point; rayStart: start point of ray; rayBefore: point have been selected
		//computation the rayLine

		std::vector<std::vector<double>> raySourceStore;
		for (int i = 0; i < rayEdageBound.size(); i++) {
			std::vector<double> rayStart_i = rayEdageBound[i];
			if (rayStart_i[0] == rayStart[0] && rayStart_i[1] == rayStart[1]) {
				std::vector<double> raySource_i;
		...
		if (raySourceStore.size() == 1) {
			...
			std::vector<std::vector<double>> raySourceStore2;
			for (int i = 0; i < raySourceStore.size(); i++) {
				std::vector<double> raySourceStore_i = raySourceStore[i];
				bool judge = true;
...
void VoronoiD();
void Voronoi2D(std::vector<std::vector<double>> points);
