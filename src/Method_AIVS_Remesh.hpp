...
public:
	...
	void AIVS_Reconstruct_Remesh(int pointNum, const std::string& fileName) {

		std::cout << "AIVS_Reconstruct start." << std::endl;
		std::vector<std::vector<double>> seedPoints;
		simplification_Method_AIVS_OM sma;

		//0. simplification
		if (br.pointCloudData.size()< (pointNum * 3)) {
			std::cout << "0. AIVS_Reconstruct up-sampling:" << std::endl;
			std::vector<std::vector<std::vector<double>>> pN = AIVS_Reconstruct_UpPointsTran(br.pointCloudData);
			std::vector<int> borderIndex = AIVS_Reconstruct_BorderReturn(pN[0]);
			BallRegion brUp;
			brUp.BallRegion_init(pN[0], pN[1], borderIndex);
			std::cout << "1. AIVS_Reconstruct Simplification:" << std::endl;
		...
		//2. Meshing
		std::cout << "2. AIVS_Reconstruct Meshing:" << std::endl;
		CGAL_Advancing_Reconstruct cr;
		cr.CGAL_Advancing_Remesh_init(seedPoints);
		std::vector<std::vector<int>> faceInfor = cr.CGAL_Advancing_Remesh_FaceInfor();		

		//3. Check the wrong trangular		
		std::cout << "3. AIVS_Reconstruct Face Detect:" << std::endl;
		std::vector<std::vector<int>> faceInforN = AIVS_Reconstruct_FaceRemove(faceInfor, seedPoints);

		//3. Store FIle	
		std::cout << "4. AIVS_Reconstruct Store File:" << std::endl;
		std::filesystem::path fn {fileName};
		std::string offName = fn.stem().string() + ".off";
		std::string objName = fn.stem().string() + ".obj";
		AIVS_SaveOFF(offName, seedPoints, faceInforN);
		AIVS_SaveOBJ(objName, seedPoints, faceInforN);
		std::cout << "AIVS_Reconstruct finish." << std::endl;
	}
...
private:
	...
	std::vector<std::vector<std::vector<double>>> AIVS_Reconstruct_IncreasePoints(std::vector<std::vector<double>> p, int Upara) {
		
		//Upara is the increase parameter		
		double UparaStep = (double)1 / (double)(Upara * 2 + 1);
		int IterNum0 = p.size();
		int GlobalIter = 2;
		std::vector<std::vector<double>> pNN = p;
		while (GlobalIter > 0) {
			GlobalIter--;
			pcl::KdTreeFLANN<pcl::PointXYZ> kdtreeSeed = AIVS_KdTreeConstruct(p);//kdtree
			std::vector<std::vector<double>> pAdd;
			int kn = 8;
			for (int i = 0; i < pNN.size(); i++) {
				if (i % 1000 == 0) {
					std::cout << (pNN.size() - i) / 1000 << " ";
				}
				std::vector<double> seedPoints_i = pNN[i];
				...
				for (int j = 1; j < pointIdxNKNSearch.size(); j++) {
					...
					std::vector<double> pNeiborj = p[indexij];
					for (int k = 0; k < Upara; k++) {
						std::vector<double> pNew_ijk(3);
		...
		std::vector<std::vector<double>> pNormal(p.size());

#pragma omp parallel for
		for (int i = 0; i < p.size(); i++) {
			if (i % 1000 == 0) {
				std::cout << (p.size() - i) / 1000 << " ";
			}
			std::vector<double> pNewi =  AIVS_pointUpdate(p[i]);
			p[i][0] = pNewi[0];
			p[i][1] = pNewi[1];
			p[i][2] = pNewi[2];
			std::vector<double> pNi(3);
			pNi[0] = pNewi[3];
			pNi[1] = pNewi[4];
			pNi[2] = pNewi[5];
			pNormal[i] = pNi;		
		...
		std::vector<std::vector<double>> pRemoveRepeat;
		std::vector<std::vector<double>> pRemoveRepeatN;

		int kn = 2;		
		pcl::KdTreeFLANN<pcl::PointXYZ> kdtreeSeed = AIVS_KdTreeConstruct(p);//kdtree
		for (int i = 0; i < p.size(); i++) {

			std::vector<double> seedPoints_i = p[i];
			std::vector<int> pointIdxNKNSearch(kn);
			std::vector<float> pointNKNSquaredDistance(kn);
			std::vector<int> pointNeibor_i;
			pcl::PointXYZ searchPoint;
	...
	std::vector<std::vector<std::vector<double>>> AIVS_Reconstruct_UpPointsTran(std::vector<std::vector<double>> p) {

		std::cout << "increase the points number from the point cloud" << std::endl;

		//1. achieve mesh
		CGAL_Advancing_Reconstruct crt;
		crt.CGAL_Advancing_Remesh_init(p);
		std::vector<std::vector<int>> faceInfor = crt.CGAL_Advancing_Remesh_FaceInfor();

		//2. achieve regular area
		std::vector<int> faceInfor1 = faceInfor[1];
		...
		for (int i = 0; i < faceInfor.size(); i++) {
			std::vector<int> faceInfor1 = faceInfor[i];
			...
			std::vector<std::vector<double>> pnewPoint = AIVS_Reconstruct_NewPoint(bi, N);
...
#pragma omp parallel for
	...
	double AIVS_Reconstruct_AchieveArea(std::vector<std::vector<double>> vPoint) {
	...
	std::vector<std::vector<double>> AIVS_Reconstruct_NewPoint(std::vector<std::vector<double>> vPoint, int N) {
		//N is area parameter
		std::vector<std::vector<double>> newPoint;
		...
		std::vector<std::vector<double>> pMiddlePoints;
		for (int i = 0; i < b1.size(); i++) {
			int seg = i + 2;
			std::vector<double> b1i = b1[i];
			std::vector<double> b2i = b2[i];
			for (int j = 0; j < i + 1; j++) {
				double weightij = (double)(j + 1) / (double)seg;
				std::vector<double> newij(3);
	...
	std::vector<int> AIVS_Reconstruct_BorderReturn(std::vector<std::vector<double>> p) {
	...
	std::vector<std::vector<int>> AIVS_Reconstruct_FaceRemove(std::vector<std::vector<int>> faceInfor, std::vector<std::vector<double>> seedPoints) {

		std::vector<std::vector<int>> faceInforN;
		for (int i = 0; i < faceInfor.size(); i++) {	
	...
	pcl::KdTreeFLANN<pcl::PointXYZ> AIVS_KdTreeConstruct(std::vector<std::vector<double>> seedpoints) {
	...
	std::vector<double> AIVS_pointUpdate(std::vector<double> point_i) {
...
#pragma region Achieve Neibor
		...
		br.pcl_kdtree.nearestKSearch(searchPoint, K, pointIdxNKNSearch, pointNKNSquaredDistance);
		...
		std::vector<std::vector<double>> pointNormal_i(pointNeior.size());
		pointNormal_i[0] = br.pointNormal[pointNeior[0]];
		for (int j = 1; j < pointNeior.size(); j++) {
			//std::vector<double> n_j = br.pointNormal[pointNeior[j]];
...
#pragma region MLS error
		...
		//std::std::vector<int> p_neibor = br.pointNeibor[i];			
		//regularNoraml

		std::vector<double> px_store(3);
		std::vector<double> nx_store(3);
		std::vector<double> ax;//new point position
		ax.push_back(0);
		ax.push_back(0);
		ax.push_back(0);
		std::vector<double> nx;//new point normal
		nx.push_back(0);
		nx.push_back(0);
		nx.push_back(0);
		...
		double weight;
		while (iter) {
			//std::vector<double> ax = simMeasurement_cop_a(px, pointNeiborRegualrNum);
			//std::vector<double> nx = simMeasurement_cop_n(px, pointNeiborRegualrNum, pointNeiborNormalRegualrNum);
			double fenmu = 0;
			...
			//std::vector<double> axnew = simMeasurement_cop_a(px, pointNeiborRegualrNum);
			//std::vector<double> nxnew = simMeasurement_cop_n(px, pointNeiborRegualrNum, pointNeiborNormalRegualrNum);
...
#pragma endregion 
		...
		std::vector<double> finalResult(6);
		finalResult[0] = px[0];
		finalResult[1] = px[1];
		finalResult[2] = px[2];
	...
	std::vector<double> AIVS_Centorid(std::vector<double> p, std::vector<std::vector<double>> pn) {

		std::vector<double> pT = CVT_Delaunay_Return(p, pn);
		std::vector<double> pPullBack = AIVS_pointUpdate(pT);
		return pPullBack;	
	}


	//CVT coordinate transfer
	std::vector<double> CVT_NormalVector(std::vector<double> v) {
	...
	std::vector<double> CVT_Global2local(std::vector<std::vector<double>> localAxis,
		std::vector<double> localCenter, std::vector<double> pG) {

		std::vector<double> xAxis = localAxis[0];
		std::vector<double> yAxis = localAxis[1];
		std::vector<double> zAxis = localAxis[2];
	...
	std::vector<double> CVT_Local2Global(std::vector<std::vector<double>> localAxis,
		std::vector<double> localCenter, std::vector<double> pl) {
...
/* // warning: unused function and no return statement!
	std::vector<double> CVT_Point2Plane(std::vector<double> p, 
																			std::vector<double> pn,
																			std::vector<double> sourceP) {//map sourceP to the plane
...
*/
	...
	std::vector<double> CVT_Delaunay_Return(std::vector<double> p, std::vector<std::vector<double>> p_neibor) {

		std::vector<double> p_c(3);
		std::vector<double> p_n(3);
		p_c[0] = p[0];
		p_c[1] = p[1];
		p_c[2] = p[2];
		p_n[0] = p[3];
		...
		std::vector<double> zLocal = CVT_NormalVector(p_n);
		std::vector<double> yDirection(3);
		yDirection[0] = p_neibor[p_neibor.size() - 1][0];
		yDirection[0] = p_neibor[p_neibor.size() - 1][1];
		yDirection[0] = p_neibor[p_neibor.size() - 1][2];
		std::vector<double> yLocal = CVT_NormalVector(yDirection);
		std::vector<double> xLocal(3);
		...
		std::vector<std::vector<double>> localAxis;
		localAxis.push_back(xLocal);
		localAxis.push_back(yLocal);
		localAxis.push_back(zLocal);

		//2. map the point into the plane
		std::vector<std::vector<double>> planeData;
		for (int i = 0; i < p_neibor.size(); i++) {
			std::vector<double> p_neibor_i = p_neibor[i];
			std::vector<double> p_neibor_iN = CVT_Global2local(localAxis, p, p_neibor_i);
			std::vector<double> p_neibor_iN2D(2);
		...
		std::vector<std::vector<double>> planeDataStore = planeData;

		if (true) {
			std::vector<double> p1; p1.push_back(0);  p1.push_back(r);
			std::vector<double> p2; p2.push_back(-r); p2.push_back(r);
			std::vector<double> p3; p3.push_back(-r); p3.push_back(0);
			std::vector<double> p4; p4.push_back(-r); p4.push_back(-r);
			std::vector<double> p5; p5.push_back(0);	 p5.push_back(-r);
			std::vector<double> p6; p6.push_back(r);  p6.push_back(-r);
			std::vector<double> p7; p7.push_back(r);  p7.push_back(0);
			std::vector<double> p8; p8.push_back(r);  p8.push_back(r);
		...
		std::vector<double> center = vd.VoronoiD_Return_Delaunay_Mean(planeData[0], planeDataStore);
		center.push_back(0);
		std::vector<double> centerReal = CVT_Local2Global(localAxis, p, center);
		return centerReal;
	}

	//file store
	void AIVS_SaveOFF(const std::string& fileName, std::vector<std::vector<double>> points, std::vector<std::vector<int>> facet) {

		std::ofstream f1(fileName, std::ios::app);
		f1 << "OFF" << std::endl;
		f1 << points.size() << " " << facet.size() << " " << 0 << std::endl;
		...
		for (int i = 0; i < facet.size(); i++) {
			f1 << 3 << " " << facet[i][0] << " " << facet[i][1] << " " << facet[i][2] << std::endl;
		}

		f1.close();
	}

	void AIVS_SaveOBJ(const std::string& fileName, std::vector<std::vector<double>> points, std::vector<std::vector<int>> facet) {
		...
		for (int i = 0; i < facet.size(); i++) {
			f1 << "f " << facet[i][0] + 1 << " " << facet[i][1] + 1 << " " << facet[i][2] + 1 << std::endl;
		}
